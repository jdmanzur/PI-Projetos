function compymove7(img)
  ...
  Q75 = [8    6    5    8    12    20    26    31;       % Quantization matrix (75%), its a predefined matrix available on google search
      ...
      36    46    48    49   56   50   52    50 ];
                              % Input your image
  i2=rgb2gray(img);                                      % rgb to gray conversion
  figure,imshow(i2), title('Before Rclose esize');
  [l, c] = size(i2)
  i2=imresize(i2, [l/4, c/4]);                           % resize the image. more the size, more the computational complexity.
  ...
  for i=2:size(L1)                                % Read the document uploaded to understand why these steps are needed
  ...
  matrix = unique(shiftvector, 'rows');        % segregate all the unique, set of elements in shiftvector
